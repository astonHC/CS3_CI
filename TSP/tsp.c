...
int TSP_ADD_CITY(TSP_STATE* STATE, const char* NAME, int X, int Y)
{
    ...
    TSP_CITY_MAX:
        TSP_ERROR_HANDLE(CITY, TSP_ERROR_CITY, 
            "CURRENT CITY COUNT: %d, EXPECTED CITY LIMIT: %d\n", 
            STATE->CITY_COUNT, TSP_MAX_CITIES);
}
...
int TSP_NEAREST(TSP_STATE* STATE) 
{
    ...
    for(int STEP = 1; STEP < STATE->CITY_COUNT; STEP++)
    {
        ...
        for(int INDEX = 0; INDEX < STATE->CITY_COUNT; INDEX++)
        {
            ...
            if(!VISITED[INDEX] && STATE->DIST.MATRIX[CURRENT][INDEX] < NEAR_DIST)
            {
                NEAR_DIST = STATE->DIST.MATRIX[CURRENT][INDEX];
                NEAREST = INDEX;
            }
        }
    ...
    STATE->TOUR.ALGO = (int)TSP_ALGO_TYPE(STATE->ALGO);
