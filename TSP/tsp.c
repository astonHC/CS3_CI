    ...
    for(int INDEX = 0; INDEX < STATE->CITY_COUNT; INDEX++)
    {
        for(int ITERATOR = 0; ITERATOR < STATE->CITY_COUNT; ITERATOR++)
        {
            ...
            if(INDEX != ITERATOR)
            {
                ...
                // 29/09/25 - SAFELY ASSIGN A VAR TO THE EUCLIDEAN DIST OF THE CITIIES 
                // AND APPLY THE INDEXES AGAINST THAT
                //
...
int TSP_NEAREST(TSP_STATE* STATE) 
{
    ...
    BASE_DIST += STATE->DIST.MATRIX[CURRENT_CITY][0];
    ...
    TSP_HANDLE(DIST, TSP_ERROR_DIST, "\nDISTANCE COVERED %d\n", STATE->TOUR.TSP_TOTAL_DIST);
